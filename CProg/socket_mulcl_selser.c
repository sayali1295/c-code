 ...
 * Later if another fd is found then (receive) or (send) will execute
 * All fds after listening socket fd are linked to (clients) so only S/R can work
 * PN: socket_mulcl_selser.c
 * Final Update Date: 27 AUG 2026
 */
		...
		for(int i = 0; i < MAX_CL; i++)
		{
			...
			if(c_fd > 0 && FD_ISSET (c_fd, &readfds))
			{
				...
				r_read = read (c_fd, buffer, 199);
					...
					{
						buffer[r_read] = '\0';
						printf("Msg from client [FD %d, Port: %d] >>\t %s\n", c_fd, ntohs(c_address.sin_port), buffer);
