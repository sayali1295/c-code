/* This program is CLIENT 1 of multiple client handling SERVER
 * In this program client is requesting service from SERVER listening on PORT 8080
 * PN : socket_trial_cl1.c
...
int main()
{
	...
	strcpy (msg, "CLIENT 1: Hey Server! Can you tell me my info?");
	...
	int rec = recv (c_fd, &received_data, sizeof(received_data), 0);
		...
		else if (rec == sizeof(received_data))
		{
			// converting infor back in human readable format
		...
		{
			printf("Error occurred in received complete data packets\n");
		}
